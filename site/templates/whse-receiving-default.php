<?php
	...
	if ($input->get->ponbr) {
		...
		if ($warehouse_receiving->purchaseorder_exists()) {
			...
			// IF NO ITEMS are available check cobol
			if (!$purchaseorder->count_receivingitems()) {
				$page->body .= $config->twig->render('util/alert.twig', ['type' => 'danger', 'title' => 'Error!', 'iconclass' => 'fa fa-warning fa-2x', 'message' => 'There are no Items to Receive']);
				$page->body .= $html->div('class=mb-3');
				$page->body .= $html->a("href=$page->url|class=btn btn-primary", "Exit");
			} else {
				...
				if ($input->get->scan) {
					...
					if ($query_phys->count() == 1) {
						...
						if ($physicalitem->has_error()) {
							...
							// AUTOSCAN
							if ($warehouse_receiving->validate_autosubmit($physicalitem)) {
								$physicalitem->save();
								$session->redirect($warehouse_receiving->get_autosumbitURL($scan));
							}
